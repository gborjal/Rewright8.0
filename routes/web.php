...
Route::middleware(['web'])->group(function () {
	...
	Route::get('login_demo_phy', [AuthController::class,'login_demo_phy']); 
	Route::get('login_demo_patient', [AuthController::class,'login_demo_patient']); 
...
Route::get('/email/verify', [AuthController::class,'verificationNotice'])->middleware(['web'])->name('verification.notice');
Route::get('/email/verify/{id}/{hash}', [AuthController::class,'verificationVerified'])->middleware(['auth', 'signed'])->name('verification.verify');
Route::post('/email/verification-notification', [AuthController::class,'verificationNotice'])->middleware(['web', 'throttle:6,1'])->name('verification.send');
